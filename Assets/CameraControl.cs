...
public class CameraControl : MonoBehaviour {
	...
	private void Awake ()
	{
		// Try to find objects tagged as players.

		player1 = GameObject.FindGameObjectWithTag ("Player1");
		player2 = GameObject.FindGameObjectWithTag ("Player2");
		m_Camera = GetComponentInChildren<Camera> ();
		if (player1 != null) {
			Debug.Log ("Found GameObject tagged Player1!");
		}
		m_Camera = GetComponentInChildren<Camera> ();
		if (player2 != null) {
			Debug.Log ("Found GameObject tagged Player2!");
		}
	...
	private void Update()
	{
		...
		// Change the size of the camera as long as maximum distance between players aren't reached.
	...
	private void Move ()
	{
		...
		// If maximum distance is reached, start following the player in lead 
		...
		// transform.position = Vector3.MoveTowards(transform.position, m_DesiredPosition, step);
		 ...
		 transform.position = Vector3.SmoothDamp (transform.position, m_DesiredPosition, ref m_MoveVelocity, m_DampTime);
			...
			// Method to calculate camera position when following leader only.
	private void followLeader(Transform leader){
		
		Vector3 averagePos = new Vector3 ();
		averagePos = new Vector3(leader.position.x-4, transform.position.y, transform.position.z);
		m_DesiredPosition = averagePos;

	}

			// Method to calculate camera position when following both players.
